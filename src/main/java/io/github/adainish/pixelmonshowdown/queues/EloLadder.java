...
public class EloLadder {
    ...
    public void loadLadder(){
        try{
            if(DataManager.getElosNode().node("Player-Elos", formatName).isMap()) {
                DataManager.getElosNode().node("Player-Elos", formatName).childrenMap().forEach((uuid, stats) -> {
            ...
            PixelmonShowdown.getInstance().log.error("PixelmonShowdown has encountered an error loading elo profiles!");
            e.printStackTrace();
        }
    }
    ...
    public void addAsActive(UUID player){
        ...
        if(profile != null && !activeEloProfiles.contains(profile)){
    ...
    //Get a high to low sorted list of eloprofiles
    public List<EloProfile> getProfiles()
    {
        List<EloProfile> sortedList = new ArrayList<>(eloProfilesByElo);
        sortedList.sort(Comparator.comparing(EloProfile::getElo));
        return sortedList;
    }
